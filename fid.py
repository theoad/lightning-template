...
class FID(Metric):
    ...
    def update(self, img: Tensor, target: Optional[Tensor] = None, real: Optional[bool] = False) -> None:  # type: ignore
        ...
        if self.real_prepared and real:
            pass
        if self.to_255:
            img = (255 * (img - self.data_low) / self.data_range).type(torch.uint8)
